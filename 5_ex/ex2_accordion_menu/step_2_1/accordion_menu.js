...
$(document).ready(function(){
	$(".menu_item").bind("mouseenter", function(){
		...
		if($currentActiveMenuItem){
			...
			$currentActiveMenuItem.animate({height:DE_ACTIVE_HEIGHT},
						ANIMATION_DURATION,
						"easeOutQuint"
			);
			
			//1-2. 메뉴 아이템의 컨텐츠 위치 값을 0으로만든다.
		...
		$currentActiveMenuItem.stop();
		$currentActiveMenuItem.animate({height:ACTIVE_HEIGHT}, 
			ANIMATION_DURATION,
			"easeOutQuint"
		);
		
		//2-2. 메뉴 아이템 컨텐츠 위치 값을 -20만큼 이동시킨다
		// (메뉴 아이템 활성화 영역이 보이도록)
		$currentActiveMenuItem.find("div").stop();
		$currentActiveMenuItem.find("div").animate({top:-DE_ACTIVE_HEIGHT}, 
			ANIMATION_DURATION,
			"easeOutQuint"
		);
	});
