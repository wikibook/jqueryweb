...
$(document).ready(function(){
	...
	// image_gallery.xml파일 읽기.
	loadData("image_gallery.xml");	
});
...
function init(){
	...
	this.$imageComment = $("#image_comment");
}
...
function loadImage(nIndex){
	...
	// comment정보 업데이트 시키기.
	this.updateImageComment(nIndex);
	
	// 현재 활성화된 이미지 인덱스 값 업데이트 시키기.
...
// 이전, 다음 이미지 이동에 따른, 페이지 이동처리하기.
function updatePageIndex(nImageIndex){
	// nImageIndex 값이 몇 페이지에 속하는 이미지인지 알아낸다.
	var nPageIndex = Math.floor(nImageIndex/this.PAGE_THUMB_COUNT);	
	
	if(this.nCurrentPageIndex!=nPageIndex)
		this.showPage(nPageIndex);
	
}

// image_gallery.xml 읽어들이기.
function loadData(strFileName){
	// 로딩 패널 보이기.
	this.showLoadingPanel(true);
	
	
	$.get(strFileName, null, function(data){
	
		
		// 동적으로 썸네일 목록 생성하기.
		createThumbnaiList(data);
...
// 썸네일 리스트 생성하기.
function createThumbnaiList(strData){
	
	// 이미지 데이터 저장.
	this.$imageModel = $(strData).find("image");
	
	
	// 문자열로  <img> 태그 목록 생성하기.	
	var strImageList = "";
	this.$imageModel.each(function(index){
		strImageList += "<img src='" + $imageModel.eq(index).find("thumb").text() + "'>";
	});
	
	// 생성한 썸네일 목록 문자열을  썸네일 영역에 추가하기.
	this.$thumbList.append(strImageList);
}
...
function start(){
	...
	// 이벤트 초기화시키기.
	initEventListener();
	// 시작시 첫 번째 이미지를 선택된 이미지로 만듬.
	loadImage(0);
}

// 외부에서 읽어들인 이미지 외부 데이터에서 comment정보를 읽어 #imageComment에 출력하기. 
function updateImageComment(nImageIndex){
	this.$imageComment.html(this.$imageModel.eq(nImageIndex).find("comment").text());
}
